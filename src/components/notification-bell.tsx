...
export function NotificationBell({ userId, initialCount }: { userId: string; initialCount: number }) {
  ...
  return (
    <Link
      ...
      className="relative ml-auto flex size-11 shrink-0 items-center justify-center rounded-full border-2 border-ink bg-paper shadow-brutal-sm nb-press-sm"
    ...
    >
      ...
      {count > 0 ? (
        ...
        <span className="absolute -top-1.5 -right-1.5 flex min-w-5 items-center justify-center rounded-full border-2 border-ink bg-coral px-1 text-[10px] font-semibold tabular-nums leading-4 text-ink">
          {count > 99 ? "99+" : count}
        </span>
      ) : null}
    </Link>
